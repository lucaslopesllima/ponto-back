...
import {
  ManualPunchRequestModel,
  ManualPunchRequestStatus,
} from '../models/manual-punch-request.model.js';
import { TimeEntryModel, type PunchKindValue } from '../models/time-entry.model.js';
...
export async function createManualPunchRequest(
  userId: string,
  timeZone: string,
  input: { timestamp: string; reason: string }
) {
  ...
  if (requested.getTime() >= now - SKEW_MS) {
    throw new HttpError(
      400,
      'Só é permitido solicitar horário retroativo (anterior ao momento atual).',
      'NOT_RETROACTIVE'
    );
  }
  ...
  const kind: PunchKindValue = nextExpectedKind(before);
  ...
  if (pendingDup) {
    throw new HttpError(
      400,
      'Já existe uma solicitação pendente para este horário.',
      'DUPLICATE_PENDING'
    );
  }
...
export async function listMyManualPunchRequests(userId: string) {
...
export async function listPendingManualPunchRequests(
  adminId: string,
...
export async function decideManualPunchRequest(
  adminId: string,
  requestId: string,
  approve: boolean
) {
