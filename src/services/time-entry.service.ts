...
import {
  ...
  PunchKind,
  type PunchKindValue,
} from '../models/time-entry.model.js';
...
export function normalizePunchKind(raw: string): PunchKindValue {
  if (raw === 'ENTRADA' || raw.startsWith('ENTRADA')) return PunchKind.ENTRADA;
  if (raw === 'SAIDA' || raw.startsWith('SAIDA')) return PunchKind.SAIDA;
...
export async function getTodayEntries(
  userId: string,
  timeZone: string
): Promise<{ kind: PunchKindValue; timestamp: Date }[]> {
...
export function nextExpectedKind(
  entries: { kind: PunchKindValue }[]
): PunchKindValue {
...
): Promise<{ id: string; timestamp: Date; address: string | null }> {
...
function buildDayRow(
  dateStr: string,
  entries: { type: string; timestamp: Date; address?: string | null }[],
  user: IUser
): DayMirrorRow {
  ...
  const fmt = (d: Date) => formatDateInTz(d, tz, 'HH:mm');
  ...
  for (let i = 0; i + 1 < sorted.length; i += 2) {
    ...
    if (b > a) totalMinutes += (b.getTime() - a.getTime()) / 60000;
  }
  ...
  let status: DayMirrorStatus = 'NORMAL';
  if (n >= 4) {
    status = 'CONCLUIDA';
  } else if (n === 0) {
    ...
    if (actualMin > expectedMin + tol) status = 'ATRASO';
  }
...
function isWeekendInUserTz(dateStr: string, tz: string): boolean {
  const [yy, mm, dd] = dateStr.split('-').map(Number);
  const midDay = new Date(Date.UTC(yy, mm - 1, dd, 15, 0, 0));
...
async function applyMirrorVisibilityFilters(
  rows: DayMirrorRow[],
  user: IUser,
  holidayYear: number
): Promise<DayMirrorRow[]> {
  const ws = user.workSchedule ?? {};
  const hideWeekends = ws.worksWeekends === false;
  const hideHolidays = ws.worksHolidays === false;
  if (!hideWeekends && !hideHolidays) return rows;
  ...
  return rows.filter((row) => {
    ...
    if (hideWeekends && isWeekendInUserTz(row.date, tz)) return false;
    if (hideHolidays && holidaySet?.has(row.date)) return false;
    return true;
  });
}

export async function getMirrorMonth(
  user: IUser,
  year: number,
  month: number // 1-12
): Promise<DayMirrorRow[]> {
  ...
  const byDay = new Map<
    string,
    { type: string; timestamp: Date; address?: string | null }[]
  >();
...
export async function getMirrorDateRange(
  user: IUser,
  fromYmd: string,
  toYmd: string
): Promise<DayMirrorRow[]> {
...
export async function listEntriesRaw(userId: string, from: Date, to: Date) {
