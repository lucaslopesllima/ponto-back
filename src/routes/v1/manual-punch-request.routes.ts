...
import {
  ...
  listMyManualPunchRequests,
  listPendingManualPunchRequests,
  decideManualPunchRequest,
} from '../../services/manual-punch-request.service.js';
...
const pendingListQuerySchema = z.object({
  ...
  limit: z.preprocess(
    ...
    z.coerce.number().int().refine(
      (n) => [0, 5, 10, 20, 50, 100].includes(n),
      'limit deve ser 0, 5, 10, 20, 50 ou 100'
    )
  ),
});
...
export async function manualPunchRequestRoutes(app: FastifyInstance): Promise<void> {
  app.post(
  ...
  app.post(
    '/manual-punch-requests/:id/decide',
    { preHandler: [requireAdmin] },
    async (request, reply) => {
