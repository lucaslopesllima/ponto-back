...
import { getMirrorMonth, type DayMirrorRow } from '../../services/time-entry.service.js';
...
function toCsv(rows: DayMirrorRow[]): string {
...
async function toXlsx(rows: DayMirrorRow[]): Promise<Buffer> {
  ...
  for (const r of rows) {
    const rowVals: (string | number)[] = [r.date];
    for (let i = 0; i < maxP; i++) rowVals.push(r.times[i] ?? '');
...
export async function reportsRoutes(app: FastifyInstance): Promise<void> {
  app.get(
    ...
    async (request, reply) => {
      ...
      reply.header(
        'Content-Disposition',
        `attachment; filename="espelho-${q.year}-${String(q.month).padStart(2, '0')}.csv"`
      );
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      reply.header(
        'Content-Disposition',
        `attachment; filename="espelho-${q.year}-${String(q.month).padStart(2, '0')}-${reportUser.name.replace(/\s+/g, '-').toLowerCase()}.xlsx"`
      );
